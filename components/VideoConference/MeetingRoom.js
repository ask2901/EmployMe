...
const MeetingRoom = ({ route, navigation }) => {
  ...
  const onJoinMeeting = () => {
    Alert.prompt(
      ...
      (id) => {
        ...
        navigation.navigate("Room", { itemId: id });
      ...
      <View style={styles.wrapper}>
        {rooms.map((room, index) => (
          <Link href={`/(inside)/(room)/${room.id}`} key={index} asChild>
            <TouchableOpacity>
              <ImageBackground key={index} source={room.img} style={styles.image} imageStyle={{ borderRadius: 10 }}>
